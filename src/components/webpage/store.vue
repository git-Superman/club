<template>
    <div class="store">
        <myTitle center="户外商城" :right="icon"></myTitle>
        <!-- <main class="images">
        ...
        <div class="list">
            <router-link :to="'/store/this/'+item.id" v-for="(item,i) in list" :key="i" class="list-item">
                <div>
                    <img v-if="item.logo != imgURL" :src="item.logo" :alt="item.name">
                    <p>LOGO</p>
                    <span>{{item.name}}</span>
                </div>
            </router-link>
...
import myTitle from '@/views/title'
    ...
    export default {
        ...
        methods:{
            ...
            init(){
                ...
                this.axios.post('/Shop/GetCategoryFirst').then(res=>{
                    if(res.code == 0){
                        var data = res.data;
                        data.forEach(item => {
                            item.logo = this.imgURL + item.logo;
                        });
                        this.list = data;
                    }else{
                        this.$toast(data.msg);
